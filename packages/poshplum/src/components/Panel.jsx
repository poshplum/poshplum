...
const Icon = Layout.namedSlot("Icon").withMarkup(
    ({ className = "", "aria-label": ariaLabel, children, ...moreProps }) => {
        ...
        return (
            <span {...ariaAttrs} className={`小簡 ${className}`} {...moreProps}>
                {children}
            </span>
        );
    }
...
const HeaderRight = Layout.namedSlot("HeaderRight").withMarkup(
    ({ className = "", children, ...moreProps }) => (
        <div className={`float-right ${className}`} {...moreProps}>
            {children}
        </div>
    )
);
...
const Footer = Layout.namedSlot("Footer").withMarkup(
...
const FixedHeader = Layout.namedSlot("FixedHeader").withMarkup(
    ({ children, ...props }) => {
        const err = Object.keys(props).length ? (
            <div className="alert alert-danger">
                FixedHeader doesn't accept extra props.
            </div>
        ) : (
            ""
        );
...
export class Panel extends Layout {
    ...
    componentDidMount() {
        let input = this.node.current.querySelector(
    ...
    render() {
        ...
        if (withRef && this.node !== withRef)
            throw new Error(
                `Panel: assertion failure: withRef= isn't expected to be changing`
            );
